    ...
    <div style="margin-left: 20px">
      ...
      <a-row>
        <a-col :span="24">
          ...
          <img alt="example" style="width: 20%;height:20%;margin: 10px" v-for="item in exceptionImageList" :src="item"
        ...
        <a-row>
          ...
          <a-col :span="24" v-if="form.handleType == 1">
            <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="追加金额" prop="amount">
              <a-input-number v-model="form.amount" :min="0" @change="onChangeAmount"/>
              <span class="label-content">&nbsp;元</span>
            </a-form-model-item>
          </a-col>
          <a-col :span="24" v-if="form.handleType == 2">
            <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="退款金额" prop="refundType">
              <a-radio-group v-model="form.refundType">
                <a-radio value="1">整单退款</a-radio>
                <a-radio value="2" v-if="!['self', 'site_self'].includes(orderType)">仅退回商品金额</a-radio>
              </a-radio-group>
            </a-form-model-item>
          </a-col>
        </a-row>
      ...
      <a-row>
        ...
        <a-col :span="4">
          <a-button type="primary" @click="sendAdditionalOrder" size="large">确认</a-button>
        </a-col>
        <a-col :span="8"></a-col>
      </a-row>
    </div>
...
export default {
  ...
  methods: {
    onChangeAmount(value) {
      this.form.amount = parseFloat(`${parseFloat(value + 0).toFixed(2)}`) || 0
    },
    show(record) {
      this.visible = true;
