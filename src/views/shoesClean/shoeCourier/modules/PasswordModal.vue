    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="用户账号" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input placeholder="请输入用户账号" v-decorator="[ 'phone', {}]" :readOnly="true"/>
        </a-form-item>
...
<script>
  import {changeCourierPassword} from '@/api/api'
  ...
  export default {
    ...
    methods: {
      show (phone, courierId) {
        this.form.resetFields();
        this.visible = true;
        this.model.phone = phone;
        this.model.courierId = courierId;
        this.$nextTick(() => {
          this.form.setFieldsValue({phone:phone});
        });
      },
      close () {
        this.$emit('close');
      ...
      handleSubmit () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            changeCourierPassword(formData).then((res)=>{
              if(res.success){
