  ...
  >
    <div class="diyDiv">
      ...
      <a-descriptions title="订单信息" layout="vertical" bordered :column="6" size="small" style="margin-bottom: 20px">
        ...
        <a-descriptions-item label="机柜名称-格子数" v-if="statusInt > 0 && statusInt < 5 && '快递上门' !== data.type">
          {{ data.lockerName }}-{{ data.beforeGridNo }}
        </a-descriptions-item>
        <a-descriptions-item label="机柜名称-格子数" v-if=" '快递上门' !== data.type && statusInt >= 9 && statusInt <= 13 &&  ('上门取件' === data.type || '站点自寄' === data.type)">
