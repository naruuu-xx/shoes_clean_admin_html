...
<!--      <a-button type="primary" icon="download" @click="handleExportXls('shoe_timecard_record')">导出</a-button>-->
...
<!--    </div>-->
    ...
    <div>
      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
              ...
              <a-menu-item v-if="record.timecardStatus=='1' && record.useStatus != '已使用'">
                <a-popconfirm title="确定停用吗?" @confirm="() => handleDelete(record.shoeTimecardRecordId)">
                  <a>停用</a>
                </a-popconfirm>
              </a-menu-item>
                  <a v-if="record.timecardStatus=='0'" style="color: black" >已停用</a>
    ...
    <shoe-timecard-record-modal ref="modalForm" @ok="modalFormOk"></shoe-timecard-record-modal>
  </a-card>
</template>
...
<script>
  ...
  import ShoeTimecardRecordModal from './modules/ShoeTimecardRecordModal'
  import {deleteAction} from "@api/manage";

  export default {
    name: 'ShoeTimecardRecordList',
    mixins:[JeecgListMixin, mixinDevice],
    components: {
      ShoeTimecardRecordModal
    },
    data () {
      return {
        ...
        description: 'shoe_timecard_record管理页面',
        // 表头
        columns: [
          // {
          //   title: '#',
        ...
        url: {
          list: "/shoeTimecardRecord/list",
          delete: "/shoeTimecardRecord/delete",
          deleteBatch: "/shoeTimecardRecord/deleteBatch",
        },
        dictOptions:{},
        superFieldList:[],
        selectionRows: [],
    ...
    methods: {
      ...
      getSuperFieldList(){
        let fieldList=[];
        fieldList.push({type:'int',value:'timecardRecordId',text:'优惠券发放记录ID',dictCode:''})
        fieldList.push({type:'int',value:'timecardId',text:'优惠券id',dictCode:''})
        fieldList.push({type:'int',value:'userId',text:'用户id',dictCode:''})
        fieldList.push({type:'int',value:'userTimecardId',text:'用户优惠券id',dictCode:''})
